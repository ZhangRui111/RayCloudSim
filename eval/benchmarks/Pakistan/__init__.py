...
- TaskSize:      Input data size in kilobytes (KB).
- CyclesPerBit:  The CPU cycles needed per bit of task data.
- TransBitRate:  Transfer bit rate (in kbps) used when offloading the task.
...
------------------------------------------------------------------------------
1. CPU Frequency (cpu_freq in MIPS/1000)
------------------------------------------------------------------------------
    - MIPS/1000 translates to "thousands of instructions per second."
    - The value represents the product of (number_of_cores) * (frequency_per_core).
      ...
      second, then:
          cpu_freq = 4 * 2,500 = 10,000 MIPS/1000
      This means the node can handle 10,000 * 1,000 = 10 million instructions 
      per second in total.
...
------------------------------------------------------------------------------
  ...
  - MaxCpuFreq:      Maximum CPU frequency in MIPS/1000 (thousands of instructions 
                     per second). Reflects aggregated core frequencies.
  - MaxBufferSize:   The maximum number of tasks (or size of task queue) that can 
...
------------------------------------------------------------------------------
  ...
  - Bandwidth:   The maximum link capacity in kilobits per second (kbps). Actual
