...
class BaseScenario(metaclass=ABCMeta):
    ...
    def calculate_base_latency(self, edge_info: dict, 
                               src_node_id: int, dst_node_id: int, nodes: dict) -> float:
        """
        ...
        if src_node.location and dst_node.location:
            distance = src_node._calculate_distance(dst_node) * 2  # Round trip distance in meters
